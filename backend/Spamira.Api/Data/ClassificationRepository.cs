...
public sealed class ClassificationRepository(SpamiraDbContext db) : IClassificationRepository
{
    ...
    public Task<ClassificationResult?> GetAsync(Guid id, CancellationToken ct) =>
        db.Classifications.AsNoTracking().SingleOrDefaultAsync(x => x.Id == id, ct);
    ...
    {
        var query = db.Classifications.AsNoTracking();
    ...
    {
        var counts = await db.Classifications.GroupBy(x => x.Label)
            .Select(g => new { Label = g.Key, Count = g.Count() }).ToListAsync(ct);
        var recent = await db.Classifications.AsNoTracking().OrderByDescending(x => x.CreatedAt).ThenByDescending(x => x.Id).Take(5).ToListAsync(ct);
